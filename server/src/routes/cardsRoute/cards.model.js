//cards.model.js handels all the reading, writing, and manipulation of data for the card API.

const path = require("path"); //The module that allows to navigate directories agnostic of OS.
const fs = require("fs"); //The tool that will allow is to read and write to the drive.
const masterBingoCardList = require("../../bingo/masterList/bingoList"); //The object where all challenges are stored.

const { destroyArrayImage } = require("../imageRoute/image.model"); //When we destory a card we need to destory all the images associated with it. We import this function so that we can delete images from the drive.

//Each card is made of indivdual cells. This object represents a card cell.
class BingoCellData {
...
//Rather than repeating this code every function we are going to put it here at the top of the scope for this module.
...
//Needed in generating a card with challenges in a random order.
...
function saveList(cardType, cardName, cardData) {
  try {
    const cards = readJsonFromFile(bingoCardPath);
    ...
    if (Array.isArray(cardData)) {
      ...
      writeJSONToFile(bingoCardPath, cards);
...
function findAllListNames() {
  try {
    const jsonData = fs.readFileSync(bingoCardPath, "utf-8");
...
//export functions so that cards.controller.js can use them
module.exports = {
